...
import { createLeadSchema, updateLeadSchema } from '../server/validation.js';
...
app.get('/api/leads', isAuthenticated, async (req, res) => {
  try {
    ...
    const mapped = data.map((l) => ({
      id: l.id, name: l.name, email: l.email, budgetRange: l.budget_range,
      message: l.message, status: l.status, createdAt: l.created_at,
    }));
...
app.get('/api/leads/stats', isAuthenticated, async (req, res) => {
  try {
    ...
    data.forEach((l) => { if (byStatus[l.status] !== undefined) byStatus[l.status]++; });
...
app.patch('/api/leads/:id', isAuthenticated, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('leads').update({ status: parsed.data.status }).eq('id', id).select().single();
    ...
    res.json({
      success: true, lead: {
        id: data.id, name: data.name, email: data.email, budgetRange: data.budget_range,
        message: data.message, status: data.status, createdAt: data.created_at,
      },
    });
  } catch (err) {
    console.error('Server error:', err);
